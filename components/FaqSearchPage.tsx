...
function QuestionRow({ entry, open, onToggle }: { entry: FaqEntry; open: boolean; onToggle: () => void }) {
  return (
    <div
      className={`border-b border-ink/[.08] last:border-b-0 border-l-[3px] ${redFlagStyle[entry.redFlag]}`}
    >
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        ...
        <span className="flex-1 font-medium text-[15.5px] leading-[1.6]">
          {entry.question}
        </span>
        {entry.redFlag === "urgent" && (
          <span className="shrink-0 text-[11px] font-medium px-2.5 py-1 rounded-md bg-[#fbe7e4] text-alert">
            立即回診
          </span>
        )}
      ...
      {open && (
        <div className="px-4 sm:px-[22px] pb-6 pl-[52px] animate-ds-fade">
          <p className="text-[15.5px] leading-[1.9] font-medium text-ink mb-3">
            {entry.shortAnswer}
          </p>
          {entry.detailAnswer && (
            <div className="text-[14.5px] text-ink-2 font-light border-t border-ink/[.08] pt-3.5">
    ...
    <div>
      <div className="max-w-[46em] mb-6">
        <input
          ...
          className="w-full box-border text-[15.5px] px-5 py-4 border-[1.5px] border-ink/[.18] rounded-full bg-surface outline-none focus:border-navy"
        />
      </div>
