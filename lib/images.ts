...
export type Ratio = "16/9" | "3/2" | "4/3" | "3/4" | "21/9" | "1/1";
...
export const imageSlots: ImageSlot[] = [
  ...
  // ── ⑤ 日常照護 ────────────────────────────────
  {
    ...
    ratio: "21/9",
    // 圖最下緣有「守護寶寶健康髖部」「左側：股骨頭位置不良…」兩行字，不能被裁掉
    fit: "contain",
    mat: "#F8EEE2",
    alt: "保護髖關節的三要訣：包開開、抱開開、背開開",
    // 圖說寫「怎麼看這張圖」，不重述圖上已經有的字
    caption: "三格由左到右是包、抱、背；每格左邊是要避免的，右邊是建議的。",
    source: "D-004",
  },
  {
    id: "swaddle-do-dont",
    brief: "包巾對照：左為錯誤（雙腿拉直併攏綁緊），右為正確（包上不包下，下半身留空間）",
    // 2026-09-02 換成重製版 .png，是 3:2（1536×1024）。
    // 先前沿用 16/9 + cover，上下各裁掉 80px，把最上面的標題橫切成一半。
    ratio: "3/2",
    fit: "contain",
    mat: "#89512F",
    alt: "包巾的正確與錯誤包法對照",
    // 圖說拿掉：跟這一節的 lede 是同一句話，而圖上本來就寫著「正確：包上不包下」
    source: "D-001 / D-002",
  },
  {
    id: "carry-do-dont",
    ...
    mat: "#FEF1E8",
    alt: "抱姿的正確與錯誤對照",
    // 圖說拿掉：跟這一節的 lede 重複，圖上也已標「直立式抱法雙腿外展」
    source: "D-005",
  },
  {
    id: "carrier-do-dont",
    ...
    mat: "#AC6B48",
    alt: "揹巾的正確與錯誤使用對照",
    // 圖說拿掉：跟這一節的 lede 重複，圖上右欄已列「寬基底支撐／膝蓋高於臀部」
    source: "D-006 / D-007",
  },
];
